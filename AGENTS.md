...
- **Swimming** (`characters/character.gd`): the character probes the water volume each physics frame (point query at knee height). In water: movement at `SWIM_SPEED_FACTOR`, drag pulls vertical speed toward a slow sink, holding jump floats it up to the surface (`jump_held` on `CharacterController`), and a jump pressed near the surface leaps out of the water high enough to clear a bank. Verified by `tests/harness/swim_harness.tscn` (sink / float / leap-out phases with screenshots).
