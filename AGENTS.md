...
- **Macro density field**: `Helper.macro_density01(pos, world_seed)` — smooth value noise over ~`Helper.MACRO_SCALE` (144u) XZ regions, faded to 0 near the world origin so the spawn is always open meadow. Structural sockets (fill `>= 1.0`, e.g. ground laterals) ignore every field so ground stays infinite. `TerrainGenerator._effective_fill_prob` routes each `< 1.0` socket to the field treatment that fits its role:
 ...
 - **Level growth** (level-tagged piece laterals/stacking): the flatter `_level_scaled_fill` (`0.5 + 0.9*macro`) so a seeded patch spreads through the meadows the player crosses, not only a thin mid-density band. The ground topcenter SEED keeps the gentle curve, so lone meadow cliffs stay rare.
 - **Ground topcenter seeds**: the gentle legacy curve (`_gentle_scaled_fill`, `0.25 + 2.2*macro^3`); eager (`CLIFF_CORE_SEED_FILL_PROB`) inside a contour core.
 - **Everything else**: the high-contrast curve (`_macro_scaled_fill`, `0.15 + 3.2*macro^3.2`) that keeps lowlands flat.
- **Reveal margin (anti-churn, primary)**: edge tiles legitimately retile (swap variant) as their neighbours fill in — visible "appearing and disappearing in the distance" if shown while still settling, and unavoidable in the socket/variant system (the variant is a function of which neighbours exist). So generation reaches `RENDER_RANGE` (260) but tiles placed in the outer `REVEAL_MARGIN` (88u) band start `visible=false`; `_reveal_settled_pieces` (every `load_terrain`) shows them — already settled, no morph — once the player is within `RENDER_RANGE - REVEAL_MARGIN` (~172u). `register_piece` is the single chokepoint (hides on placement via `_apply_initial_visibility`; retiles re-hide because the replacement re-registers). Visibility is rendering-only — physics bodies/areas stay active, and the player never reaches the hidden band anyway. Tradeoff: visible radius ~172 vs the old 260; raising RENDER_RANGE to restore it costs ~quadratic perf (280 → 18fps in the glide harness), so the band is kept tight. Residual churn is ~40 in-place morphs/run at the very view boundary (0 pop-outs), vs ~480 events (incl. 213 pop-outs) before.
- **Doom-suppression (anti-churn, source)**: contours are deterministic, so anything a mesa will provably eat is never enqueued instead of being placed-then-removed (complements the reveal margin by cutting churn at the source). Inside a cliff core (`_in_cliff_core`, macro ≥ `CLIFF_CONTOUR_BASE`): decoration/hill sockets on a NON-cliff host return fill 0 (cliff plateau tops are exempt — the final surface, so mountains still get foliage), level fill returns 0, and `_biome_scaled_dist` ERASES the level/flat-ground/hill size+tag entries from the seed mix (erase, not zero — a 0-weight key can strand `sample_from_modules` on an unsamplable tag and make `Distribution.sample` assert in an infinite loop; `has_positive_weight()` guards the sampler too).
...
- **Support sweep**: hills and displaceable decorations whose surface vanished (e.g. the base of a hill stack removed by a bank conversion) have no rule that re-checks them; `_purge_orphaned_stacks` probes their support and removes floaters. The probe (`_has_surface_support`) checks a thin slab below the piece's AABB BOTTOM — not a fixed offset from its origin, which falsely flagged hills (origin ~1.5u above their base) as unsupported the frame they spawned (visible spawn-then-vanish). The sweep processes a rotating 64-piece slice per call (a batch-every-N version was a one-frame hitch).
- **Per-frame budgets (anti-lag-spike)**: `load_terrain` caps real placement ATTEMPTS (`num_attempts < MAX_LOAD_PER_STEP*4`) separately from cheap out-of-range deferral pops (`num_pops < MAX_LOAD_PER_STEP*8`). Counting deferrals against the attempt budget let stale low-priority out-of-range entries on the heap top consume the whole budget for many frames — nothing spawned — then the backlog landed all at once (a lag spike with everything popping in together). `_defer_if_out_of_range` sets `_last_pop_deferred` so the loop excludes those pops.
- **Player safety**: `add_piece` rejects any non-ground piece whose AABB intersects the player's body footprint (height-tracking, no origin-height exemption — hills/levels sit at ground height and used to slip through and trap the player). A rejected socket is re-deferred (not consumed) with a frame cooldown so it retries once the player moves instead of leaving a permanent bare patch or re-instantiating a candidate scene every frame.
...
- `godot --path . res://tests/harness/burst_harness.tscn` glides the character in a straight line and prints a per-frame piece-count-delta histogram plus `visible_churn` (decorations/hills removed within sight). Use it to catch two regressions: spawn bursts (a high `max_zero_streak` then a spike in `max_delta` = lag) and appear-then-disappear churn (`visible_churn` should be ~0). It also logs each `[vanish]` with the piece's macro/position so the churn source is identifiable.
- **GUT integration tests are slow** (`test_terrain_generator.gd` runs 1500+ generation iterations per test; full suite ~10min). When iterating, run a single test with `-gunit_test_name=...` (≈20s). If a run produces a multi-MB log fast, it is almost certainly the `Distribution.sample` assert-spam loop (a zero-sum sampling distribution) — kill it; do not wait.
