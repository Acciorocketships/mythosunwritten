...
- **Stale queue priorities**: priorities are distances at enqueue time. When the player stands still and the heap top is out of range, `load_terrain` re-enqueues that top at its actual distance (one per frame) instead of just returning — otherwise a far-frontier socket with a small stale priority sits on top of the heap and starves in-range pending work (deferred decorations) indefinitely.
