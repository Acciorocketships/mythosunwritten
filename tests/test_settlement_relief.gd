...
func test_the_stamp_shapes_a_grade_bench_at_the_massif_rim() -> void:
	## WAVE 1'S DESIGN EDGE, closed from the terrain side. At-grade streets are
	## contour-bound, so WarrenExcavationCarver.MIN_GRADE_CELLS (9, spread
	## MIN_GRADE_SPREAD_CELLS) is unsatisfiable on continuously sloping ground.
	## The stamp's terrace rings are the answer: the rim of the massif must land
	## on a flat run long enough and spread enough to lay that ground street on,
	## and the run must include a rim column so the portal can open onto it.
	for world_seed: int in CHAIN_CORPUS:
		var relief := _flat_relief(world_seed)
		var plan := _flat_plan(world_seed, relief)
		var region := plan.compute_region(SITE_CELL.x, SITE_CELL.y, 6)
		var bands: Dictionary = _ground_bands(region, SITE_CELL).bands
		var massif := WarrenMassifBuilder.build(world_seed, bands)
		if massif == null:
			continue
		var rim: Dictionary = {}
		for column: Vector2i in massif.columns:
			for d: Vector2i in _CARDINALS:
				if not massif.has_column(column + d):
					rim[column] = true
					break
		var footprint: Dictionary = {}
		for column: Vector2i in massif.columns:
			footprint[column] = int(bands[column])
		var run := _same_band_run(footprint, rim)
		assert_true(int(run.cells) >= WarrenExcavationCarver.MIN_GRADE_CELLS,
			"seed %d rim bench holds %d columns, %d needed" % [world_seed,
				int(run.cells), WarrenExcavationCarver.MIN_GRADE_CELLS])
		assert_true(int(run.spread)
				>= WarrenExcavationCarver.MIN_GRADE_SPREAD_CELLS,
			"seed %d rim bench spreads %d, %d needed" % [world_seed,
				int(run.spread),
				WarrenExcavationCarver.MIN_GRADE_SPREAD_CELLS])


func test_the_excavation_grade_phase_clears_on_stamped_terrain() -> void:
	## End to end, through the real carver: the bench is only worth anything if
	## the bore actually lays its ground street on it. Reported as supply against
	## the flat-ground baseline rather than tuned to, per the standing rule.
	var stamped_carves := 0
	var flat_carves := 0
	var readings := ""
	for world_seed: int in CHAIN_CORPUS:
		var relief := _flat_relief(world_seed)
		...
		if massif != null:
			...
			if excavation != null:
				stamped_carves += 1
				var reading := _grade_reading(massif, excavation)
				assert_true(int(reading.cells)
						>= WarrenExcavationCarver.MIN_GRADE_CELLS,
					"seed %d carved with only %d grade cells" % [world_seed,
						int(reading.cells)])
				assert_true(int(reading.spread)
						>= WarrenExcavationCarver.MIN_GRADE_SPREAD_CELLS,
					"seed %d grade street spread %d" % [world_seed,
						int(reading.spread)])
				readings += " %d:%d/%d" % [world_seed, int(reading.cells),
					int(reading.spread)]
		var flat := WarrenMassifBuilder.build(world_seed, {})
		if flat != null and WarrenExcavationCarver.carve(world_seed,
				flat) != null:
			flat_carves += 1
	gut.p("bores on stamped terrain %d of %d (flat baseline %d); grade "
		% [stamped_carves, CHAIN_CORPUS.size(), flat_carves]
		+ "cells/spread%s" % readings)
	assert_gt(stamped_carves, 0,
		"no seed bored a route on stamped terrain")
